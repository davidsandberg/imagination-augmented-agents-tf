...
class EnvModel():
    ...
    def __init__(self, obs_shape, num_actions, nrof_time_steps):
        ...
        encoded_obs_reshaped = observation_encoder(obs_reshaped)
        shape = [-1,length ]+encoded_obs_reshaped.get_shape().as_list()[1:]
        encoded_obs = tf.reshape(encoded_obs_reshaped, shape)
        
        # Initialize state
        encoded_obs_init = encoded_obs[:,:nrof_init_time_steps,:,:,:]
        state = initial_state_module(encoded_obs_init)
        ...
        for t in range(nrof_time_steps):
            ...
            mu_hat, sigma_hat = posterior_module(mu, sigma, state, encoded_obs[:,t,:,:,:], onehot_actions[:,t,:,:,:])
            mu_list += [ mu ]
            sigma_list += [ sigma ]
            mu_hat_list += [ mu_hat ]
        ...
        self.regularization_loss = kl_divergence_gaussians(self.mu, self.sigma, self.mu_hat, self.sigma_hat)
